function res = WENO5JS(w, flux, dflux, S, dx)

% Lax-Friedrichs Flux Splitting
a = max(abs(dflux(w))); % Max of the derivative of the flux (for us, max eigenvalue)
v = 0.5*(flux(w)+a*w); % 1/2 * (f(u) + du)
u = circshift(0.5*(flux(w)-a*w),[0,-1]); % Whatever tf this means 
